...
locals {
  ...
  boot_command_aa64_auto = ["<esc><wait5>boot -s<wait><enter><wait20s>",
    ...
    "dhclient -l /tmp/dhclient.leases.em0 -p /tmp/dhclient.em0.pid em0 ; ",
    "dhclient -l /tmp/dhclient.leases.vtnet0 -p /tmp/dhclient.vtnet0.pid vtnet0 ; ",
    "cd /tmp ; fetch http://{{.HTTPIP}}:{{.HTTPPort}}/common/gpart_setup.sh ",
    "http://{{.HTTPIP}}:{{.HTTPPort}}/${var.variant}/installscript ; ",
    "export PASSWD_CRYPTED='${var.passwd_crypted}' ; ",
    "export INIT_HOSTNAME=${var.variant}-boxv0000 ; geom -t ; sleep 3 ; ",
    "<enter>bsdinstall script /tmp/installscript<enter>"]

  boot_command_aa64_chroot = ["<esc><wait5>boot -s<wait><enter><wait20s>",
    "/bin/sh<enter><wait><wait5m>",
    "mdmfs -s 100m md1 /mnt ; mdmfs -s 100m md2 /tmp ; ",
