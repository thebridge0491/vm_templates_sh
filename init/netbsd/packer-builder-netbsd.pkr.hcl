...
locals {
  ...
  boot_command_aa64_chroot = ["<spacebar><wait10>1<enter><wait1m>",
    "a<enter><wait10>a<enter><wait10>e<enter><wait10>a<enter><wait10>",
    "ksh<enter><wait10><wait10><wait10>mount_mfs -s 100m md1 /tmp ; ",
    "dhcpcd wm0 ; dhcpcd vioif0 ; cd /tmp ; ",
    "ftp http://{{.HTTPIP}}:{{.HTTPPort}}/${var.variant}/gpt_setup.sh http://{{.HTTPIP}}:{{.HTTPPort}}/${var.variant}/install.sh ; ",
    "gpt show -l sd0 ; sleep 3 ; ",
    "sh -x /tmp/gpt_setup.sh part_format ${var.vol_mgr} ; ",
    "sh -x /tmp/gpt_setup.sh mount_filesystems ${var.vol_mgr}<enter><wait30s>",
    "env MIRROR=${var.MIRROR} REL=${var.REL} DISTARCHIVE_FETCH=${var.DISTARCHIVE_FETCH} VOL_MGR=${var.vol_mgr} sh -x /tmp/install.sh run_install ${var.variant}-boxv0000 '${var.passwd_plain}'<enter><wait>"]

  boot_command_x64_chroot = ["<spacebar><wait10>1<enter><wait1m>",
    "a<enter><wait10>a<enter><wait10>e<enter><wait10>a<enter><wait10>",
    "ksh<enter><wait10><wait10><wait10>mount_mfs -s 100m md1 /tmp ; ",
    "dhcpcd wm0 ; dhcpcd vioif0 ; cd /tmp ; ",
    "ftp http://{{.HTTPIP}}:{{.HTTPPort}}/${var.variant}/gpt_setup.sh http://{{.HTTPIP}}:{{.HTTPPort}}/${var.variant}/install.sh ; ",
  ...
  boot_command     = ("aarch64" == var.MACHINE ? local.boot_command_aa64_chroot :
