...
if [ "nox11" = "$WITH_X11" ] ; then
	case "$PACKER_BUILDER_TYPE" in
	  ...
	  virtualbox-iso|virtualbox-ovf)
		pacman --noconfirm --needed -S virtualbox-guest-dkms virtualbox-guest-utils-nox ;
...
#	chown root:vboxsf /media/sf_Data0 ;
	...
	case "$PACKER_BUILDER_TYPE" in
	  qemu) echo '' ;;
	  virtualbox-iso|virtualbox-ovf)
		pacman --noconfirm --needed -S linux-lts-headers ;
		...
		pacman --noconfirm --needed -S virtualbox-guest-dkms virtualbox-guest-utils ;
		systemctl enable vboxservice ;
		rm -f $HOME_DIR/*.iso ;
		usermod -aG vboxsf packer ; usermod -aG vboxsf vagrant ;
		;;
