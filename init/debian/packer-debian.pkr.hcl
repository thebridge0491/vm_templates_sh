...
source "qemu" "qemu_x86_64" {
  ...
  boot_command       = ["<wait>c<wait>linux /live/vmlinuz ${var.boot_cmdln_options} boot=live components username=devuan text 3 textmode=1<enter>initrd /live/initrd.img<enter>boot<enter>", "<wait10><wait10><wait10><wait10><wait10><wait10>", "<wait10><wait10><wait10><wait10><wait10><wait10>", "<wait10><wait10><wait10><wait10><wait10><wait10>", "<enter>devuan<enter>devuan<enter>sudo su<enter>ip link ; sleep 3 ; dhcpcd eth0 ; dhclient eth0 ; systemctl stop ssh ; systemctl status ssh ; invoke-rc.d ssh stop ; invoke-rc.d ssh status<enter>sleep 3 ; . /etc/os-release ; mount -o remount,size=1G /run/live/overlay ; df -h ; sleep 5 ; sed -i '/main.*$/ s|main.*$|main contrib non-free|' /etc/apt/sources.list ; apt-get --yes update --allow-releaseinfo-change ; apt-get --yes install ${var.foreign_pkgmgr} gdisk lvm2 btrfs-progs ; ", "if [ 'zfs' = '${var.vol_mgr}' ] ; then . /etc/os-release ; sed -i 's|^#deb|deb|g' /etc/apt/sources.list ; apt-get --yes update ; apt-get --yes install --no-install-recommends linux-headers-$(uname -r) ; apt-get --yes install -t $${VERSION_CODENAME/ */}-backports --no-install-recommends zfs-dkms zfsutils-linux ; fi ; ", "cd /tmp ; wget -O /tmp/disk_setup.sh 'http://{{ .HTTPIP }}:{{ .HTTPPort }}/common/disk_setup_vmlinux.sh' ; wget -O /tmp/install.sh 'http://{{ .HTTPIP }}:{{ .HTTPPort }}/${var.variant}/${var.vol_mgr}-install.sh' ; env MKFS_CMD=${var.mkfs_cmd} sh -x /tmp/disk_setup.sh part_format sgdisk ${var.vol_mgr} ; sh -x /tmp/disk_setup.sh mount_filesystems ${var.vol_mgr}<enter><wait10><wait10><wait10>env RELEASE=${var.RELEASE} service_mgr=${var.service_mgr} sh -x /tmp/install.sh ${var.init_hostname} '${var.passwd_crypted}'<enter><wait>"]
