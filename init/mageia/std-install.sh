...
if command -v dnf > /dev/null ; then
  #${DNFCMD} --nogpgcheck -y --installroot=/mnt --repofrompath=quickrepo${RELEASE},http://${MIRROR}/distrib/${RELEASE}/${UNAME_M}/media/core/release/ --repo=quickrepo${RELEASE} install urpmi dnf dnf-plugins-core locales-en ;
...
pkgs_nms="basesystem kernel-desktop-latest microcode_ctl locales-en sudo whois dhcp-client man-pages dosfstools xfsprogs openssh-server nano mandi-ifw shorewall shorewall-ipv6 urpmi dnf dnf-plugins-core harddrake-ui grub grub2-efi efibootmgr" # task-xfce"
#urpmi.update -a
