...
gpart_hddisk() {
  ...
  if [ "zfs" = "$FSTYPE" ] ; then
    ...
    gpart add -a 1M -s 4G -t freebsd-swap -l fsSwap $DEVX ;
    gpart add -a 1M -s 80G -t freebsd-zfs -l zvol0 $DEVX ;
  else
    gpart bootcode -b /boot/pmbr -p /boot/gptboot -i 1 $DEVX ;
    
    gpart add -a 1M -s 4G -t freebsd-swap -l fsSwap $DEVX ;
    gpart add -a 1M -s 16G -t freebsd-ufs -l fsRoot $DEVX ;
    gpart add -a 1M -s 8G -t freebsd-ufs -l fsVar $DEVX ;
    gpart add -a 1M -s 32G -t freebsd-ufs -l fsHome $DEVX ;
    gpart add -a 1M -s 24G -t freebsd-ufs -l fsFree $DEVX ;
  fi
  
  gpart add -a 1M -s 120G -t ms-basic-data -l data0 $DEVX
  gpart add -a 1M -s 80G -t ms-basic-data -l data1 $DEVX
...
part_format_hddisk() {
  ...
  gpart_hddisk $FSTYPE ; format_partitions $FSTYPE $ZPARTNM_ZPOOLNMS
}
