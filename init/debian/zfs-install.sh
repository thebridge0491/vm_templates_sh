...
apt-get --yes update --allow-releaseinfo-change
for pkgX in linux-image-${MACHINE} grub-efi-${MACHINE} efibootmgr grub-pc-bin sudo curl tasksel bsdextrautils linux-headers-${MACHINE} dkms spl-dkms dpkg-dev ; do
  apt-get --yes install --no-install-recommends \$pkgX
done
# xfce4
tasksel install standard

DEBIAN_FRONTEND=noninteractive apt-get --yes install -t \${VERSION_CODENAME}-backports zfs-dkms
DEBIAN_FRONTEND=noninteractive apt-get --yes install -t \${VERSION_CODENAME}-backports zfsutils-linux zfs-initramfs
...
apt-mark hold zfs-dkms zfsutils-linux zfs-initramfs linux-image-${MACHINE} linux-headers-${MACHINE} linux-image-\$(uname -r) linux-headers-\$(uname -r)
#dpkg -l | grep "^hi"
