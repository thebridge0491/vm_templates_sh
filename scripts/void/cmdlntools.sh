...
if [ "$(hostname | grep -e 'box.0000')" ] ; then
  ...
  for fileX in /etc/hosts /etc/hostname /etc/rc.conf ; do
...
ln -s /etc/sv/${smtp_daemon} /etc/runit/runsvdir/default/ || true
...
firewall_tool=${firewall_tool:-nftables}
...
##done
##(sleep 120 && (iptables-nft -F ; iptables-nft -X ; ip6tables-nft -F ; ip6tables-nft -X))& && \
