...
pkgs_installed() {
  ...
  printf 'zypper --no-refresh search --type pattern\n----------\n'
  zypper --no-refresh search -t pattern ; echo ''
  printf 'zypper --no-refresh search --installed-only --type pattern\n----------\n'
  #sudo zypper --non-interactive -q refresh
  zypper --no-refresh search -i -t pattern ; echo ''
  ...
  if [ "leaf" = "${METHOD}" ] ; then
    ...
    echo "zypper locks" ; zypper locks | column -xc 78 ;
    echo "=============================" ;
