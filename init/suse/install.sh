...
#zypper --non-interactive install --no-recommends patterns-base-base
zypper --non-interactive install --no-recommends --type pattern base
# xfce laptop
...
if [ "zfs" = "$VOL_MGR" ] ; then
  ...
  zypper --non-interactive install btrfsprogs ;
