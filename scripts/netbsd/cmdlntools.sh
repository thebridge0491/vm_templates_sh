...
install_pkgs() {
  ...
  #distrosets="xbase" sh $(dirname ${0})/upgradepkgs.sh fetch_distrosets
...
EOF
  ...
  if [ -z "$(grep -e 'dbus-uuidgen --ensure' /etc/rc.local)" ] ; then
    #echo /usr/pkg/bin/dbus-uuidgen --ensure=/var/lib/dbus/machine-id >> /etc/rc.local ;
    echo /usr/pkg/bin/dbus-uuidgen --ensure >> /etc/rc.local ;
