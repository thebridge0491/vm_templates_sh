...
'Display coding lab packages (lang: {{langX}})':
  {#cmd.run:
    #- shell: /bin/sh
    - name: echo {{varsdict.distro_pkgs.get("pkgs_lang_"+langX, "")}}#}
  test.show_notification:
    - text: '{{varsdict.distro_pkgs.get("pkgs_lang_"+langX, "").split(" ")|yaml}}'
{% endfor %}
...
{% for langX in varsdict.langs.split(' ') %}
  ...
  {% set name_task = 'Install (' ~ langX ~ ') coding lab packages' %}
